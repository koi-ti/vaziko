        ...
        if ($request->ajax()) {
            ...
            if ($request->has('call')) {
                if ($request->call == 'materialp') {
                    ...
                    $data = Ordenp4::select('id', 'orden4_valor_unitario as valor', 'orden4_fh_elaboro as fecha', DB::raw("'ORD' as type"))->where('orden4_producto', $request->producto_id)->limit(10)->union($unioinpre)->union($unioncot)->orderBy('fecha', 'asc')->get();
                } else {
                    ...
                    $data = Ordenp9::select('id', 'orden9_valor_unitario as valor', 'orden9_fh_elaboro as fecha', DB::raw("'ORD' as type"))->where('orden9_producto', $request->producto_id)->limit(10)->union($unioinpre)->union($unioncot)->orderBy('fecha', 'asc')->get();
                }
            }
