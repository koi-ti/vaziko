...
class BalanceGeneralController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->has('type')) {
            ...
            //         (SELECT (saldosterceros_debito_inicial - saldosterceros_credito_inicial) FROM koi_saldosterceros WHERE saldosterceros_mes = {$mes2} AND saldosterceros_ano = {$ano2} AND saldosterceros_cuenta = koi_plancuentas.id AND saldosterceros_tercero = '202') AS inicial,
            //         (SELECT (saldosterceros_debito_mes) FROM koi_saldosterceros WHERE saldosterceros_mes = {$mes} AND saldosterceros_ano = {$ano} AND saldosterceros_cuenta = koi_plancuentas.id AND saldosterceros_tercero = '202') AS debitomes,
            //         (SELECT (saldosterceros_credito_mes) FROM koi_saldosterceros WHERE saldosterceros_mes = {$mes} AND saldosterceros_ano = {$ano} AND saldosterceros_cuenta = koi_plancuentas.id AND saldosterceros_tercero = '202') AS creditomes
            //     FROM koi_plancuentas
            //     WHERE koi_plancuentas.id IN (
            //             SELECT st.saldosterceros_cuenta
            //             FROM koi_saldosterceros AS st
            //             WHERE st.saldosterceros_mes = {$mes} AND st.saldosterceros_ano = {$ano}
            //         UNION
            //             SELECT st.saldosterceros_cuenta
            //             FROM koi_saldosterceros AS st
            //             WHERE st.saldosterceros_mes = {$mes2} AND st.saldosterceros_ano = {$ano2}
            //     )";
            //
            // // Filters
