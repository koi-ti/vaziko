...
class AsientoNif2 extends Model
{
 /**
    ...
    {
        $rules = [
            'plancuentasn_cuenta' => 'required|integer',
            'asienton2_naturaleza' => 'required'
        ];
    ...
    public static function getAsiento2($asiento)
    {
        ...
        $query->where('asienton2_asiento', $asiento);
        return $query->get();
    }
    ...
    public function store(AsientoNif $asientoNif, Array $data)
    {
        ...
        if(!$objCuenta instanceof PlanCuentaNif) {
            $response->error = "No es posible recuperar cuenta NIF, por favor verifique la información del asiento o consulte al administrador.";
            return $response;
        }
        ...
        if(!isset($data['Id']) || empty($data['Id']))
        {
            ...
            if($objCentroCosto instanceof CentroCosto)
            {
                ...
                if($objCentroCosto->centrocosto_codigo == 'OP') {
                    ...
                    $this->asienton2_ordenp = $objOrdenp->id;
                }
            }
    ...
    public static function validarAsiento2(Request $request, PlanCuenta $cuenta)
    {
        ...
        // Validar base
        if( !empty($cuenta->plancuentasn_tasa) && $cuenta->plancuentasn_tasa > 0 && (!$request->has('asienton2_base') || $request->asienton2_base == 0) ) {
        ...
        // Validar valor
        if(!$request->has('asienton2_valor') || !is_numeric($request->asienton2_valor) || $request->asienton2_valor <= 0) {
    ...
    public static function validarFacturap(Request $request)
    {
        ...
        if($facturap instanceof Facturap) {
            ...
            if($suma_valor != $request->asienton2_valor) {
                return "Las suma de los valores debe ser igual al valor del item del asiento: valor {$request->asienton2_valor}, suma $suma_valor, diferencia ".abs($request->asienton2_valor - $suma_valor);
            }
        }else{
            // Validar sucursal
    ...
    public static function validarInventario(Request $request)
    {
        ...
        if ($producto->producto_metrado == true) {
            ...
            if($request->asienton2_naturaleza == 'D') {
                ...
                // Costo salida
                $response->asienton2_valor = $costo;
            }
            ...
            // Producto normal
            if($request->asienton2_naturaleza == 'C') {
                // Recuperar prodbode
                $prodbode = Prodbode::prodbode($producto, $request->movimiento_sucursal);
                if(!$prodbode instanceof Prodbode || $request->movimiento_cantidad > $prodbode->disponibles) {
                    $response->errors = "No existen suficientes unidades para salida, unidades disponibles ".($prodbode instanceof Prodbode ? $prodbode->prodbode_cantidad  : 0).", por favor verifique la información o consulte al administrador.";
    ...
    public function movimiento(Request $request)
    {
        ...
        if($objCuenta->plancuentasn_tipo == 'P')
        {
            ...
            // Recuperar factura
            $facturap = Facturap::where('facturap1_factura', $request->facturap1_factura)->where('facturap1_tercero', $this->asienton2_beneficiario)->first();
            // Validar naturaleza
            if($request->asienton2_naturaleza == 'D') {
                if(!$facturap instanceof Facturap) {
    ...
    public function storeFacturap()
    {
        ...
        {
            $facturap = Facturap::where('facturap1_factura', $movefp->movimiento_facturap)->where('facturap1_tercero', $this->asienton2_beneficiario)->first();
    ...
    public function storeInventario()
    {
        ...
        if(!$producto instanceof Producto) {
            return "No es posible recuperar producto para la cuenta {$this->plancuentasn_cuenta} y tercero {$this->tercero_nit}, id {$this->id}, por favor verifique la información del asiento o consulte al administrador.";
        }
        ...
        if($producto->producto_unidades == true || ($producto->producto_unidades == true && $producto->producto_metrado == true))
        {
            if($this->asienton2_naturaleza == 'D') {
                // Entrada
                $costo = $this->asienton2_debito / $movfather->movimiento_valor;
                ...
                if(!$producto->producto_metrado)
                {
                    ...
                    if($costo != $this->asienton2_credito){
        ...
        if ($producto->producto_metrado == true || $producto->producto_serie == true) {
            ...
            if($movchildren->count() != $movfather->movimiento_valor) {
                return "No es posible recuperar movimientos detalle de inventario para la cuenta {$this->plancuentasn_cuenta} y tercero {$this->tercero_nit}, id {$this->id}, por favor verifique la información del asiento o consulte al administrador.";
            }
        }
        ...
        if ($producto->producto_metrado == true) {
            ...
            // Credito
                ...
                $inventario = Inventario::movimiento($producto, $movfather->movimiento_sucursal, 'AS', 0, $usalida, $this->asienton2_credito, 0);
                if(!$inventario instanceof Inventario) {
                    return $inventario;
                }
