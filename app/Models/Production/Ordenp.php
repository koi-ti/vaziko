...
class Ordenp extends BaseModel
{
    ...
    public static function getOrden ($id) {
        $query = Ordenp::query();
        $query->select('koi_ordenproduccion.*', 'cotizacion1_precotizacion', DB::raw("CONCAT(orden_numero,'-',SUBSTRING(orden_ano, -2)) as orden_codigo"), DB::raw("CONCAT(precotizacion1_numero,'-',SUBSTRING(precotizacion1_ano, -2)) as precotizacion_codigo"), DB::raw("CONCAT(cotizacion1_numero,'-',SUBSTRING(cotizacion1_ano, -2)) as cotizacion_codigo"), 'u.username as username_elaboro', 'ua.username as username_anulo', DB::raw("CONCAT(tcontacto_nombres,' ',tcontacto_apellidos) AS tcontacto_nombre"), 'tcontacto_telefono', 't.tercero_nit', DB::raw("(CASE WHEN t.tercero_persona = 'N' THEN CONCAT(t.tercero_nombre1,' ',t.tercero_nombre2,' ',t.tercero_apellido1,' ',t.tercero_apellido2) ELSE t.tercero_razonsocial END) as tercero_nombre, (CASE WHEN vd.tercero_persona = 'N' THEN CONCAT(vd.tercero_nombre1,' ',vd.tercero_nombre2,' ',vd.tercero_apellido1,' ',vd.tercero_apellido2) ELSE vd.tercero_razonsocial END) as vendedor_nombre"), 'vd.tercero_nit as vendedor_nit', 't.tercero_direccion', 't.tercero_direccion_nomenclatura', 't.tercero_municipio');
