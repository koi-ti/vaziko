/*! koi-frontend - v1.0.0 - 2017-03-17 */
!function($, window, document, undefined) {
    ...
    app.ComponentAddressView = Backbone.View.extend({
        ...
        addressChanged: function(e) {
            ...
            this.$addressField = this.$modalComponent.find("#koi_direccion"), this.$addressNomenclaturaField = this.$modalComponent.find("#koi_direccion_nm"), 
            this.$wraperSelectComponent = this.$("#render-component-select"), this.$formComponent = this.$modalComponent.find("#form-address-component"), 
            this.addressData = new Array(), this.addressDataNm = new Array(), this.num = new Array(), 
        ...
        addAddress: function(e) {
            e.isDefaultPrevented() || (e.preventDefault(), this.inputContent.val(this.$addressNomenclaturaField.val()), 
            this.$modalComponent.modal("hide"));
        },
        listeningAddress: function(e) {
            if (this.$wraperSelectComponent.empty(), parseInt($(e.target).text().trim()) > 0 || parseInt($(e.target).text().trim()) < 9) this.num = $(e.target).text().trim(), 
            ...
            this.addressDataNm.push(this.num)); else {
                this.num = [];
                for (var i = 0; i < this.validaciones.length; i++) if ($(e.target).text().trim() == this.validaciones[i]) {
                    var attributes = {
                        name: $(e.target).text().trim()
                    };
                    this.$wraperSelectComponent.html(this.templateSelect(attributes));
                }
                this.addressData[this.addressData.length - 1] != $(e.target).text().trim() ? (this.addressData.push($(e.target).text().trim()), 
                this.addressDataNm.push($(e.target).attr("data-key"))) : alertify.error("No puede seleccionar dos nomenclaturas iguales ni más de dos letras seguidas");
            }
            this.buildAddress();
        },
        ChangeSelect: function(e) {
            var _this = this;
            this.$component = this.$("div#component-input").hide(), "si" == $(e.target).val() ? (_this.$component.show(), 
            $("input#component-input-text").change(function() {
                var dato = $(this).val($(this).val().toUpperCase());
                _this.addressData.push(dato.val()), _this.addressDataNm.push(dato.val()), _this.buildAddress(), 
                _this.$wraperSelectComponent.empty();
            })) : "no" == $(e.target).val() && (_this.$component.hide(), _this.$wraperSelectComponent.empty());
        },
        removeLastItem: function(e) {
            e.preventDefault(), this.addressData.pop(), this.addressDataNm.pop(), this.buildAddress();
        },
        removeItem: function(e) {
            e.preventDefault(), this.addressData.length = 0, this.addressDataNm.length = 0, 
    ...
    app.ComponentCreateResourceView = Backbone.View.extend({
        ...
        addResource: function(e) {
            ...
            this.$inputAddress = this.$("#" + $(e.currentTarget).attr("data-address")), this.$inputCity = this.$("#" + $(e.currentTarget).attr("data-city")), 
            this.$inputEmail = this.$("#" + $(e.currentTarget).attr("data-email")), this.parameters.tcontacto_tercero = $(e.currentTarget).attr("data-tercero"), 
            _.isUndefined(this.parameters.tcontacto_tercero) || _.isNull(this.parameters.tcontacto_tercero) || "" == this.parameters.tcontacto_tercero)) return void alertify.error("Por favor ingrese cliente antes agregar contacto.");
            var _this = this, stuffToDo = {
    ...
    app.ComponentSearchContactoView = Backbone.View.extend({
        ...
        setContacto: function(e) {
            ...
            this.$inputAddress.length && this.$inputAddress.val(data.tcontacto_direccion), this.$inputCity.length && this.$inputCity.val(data.tcontacto_municipio).trigger("change"), 
...
!function($, window, document, undefined) {
    ...
    app.CreateRolView = Backbone.View.extend({
        ...
        changePermissions: function(e) {
            ...
            this.createPermisoRolView.undelegateEvents()), console.log(model), this.createPermisoRolView = new app.CreatePermisoRolView({
                model: model,
    ...
    app.ShowTerceroView = Backbone.View.extend({
        ...
        initialize: function() {
            ...
            this.rolList = new app.RolList(), this.referenceViews());
        },
        referenceViews: function() {
            this.contactsListView = new app.ContactsListView({
                collection: this.contactsList,
    ...
    app.AppRouter = new (Backbone.Router.extend({
        ...
        initialize: function(opts) {
            ...
            this.componentSearchOrdenPView = new app.ComponentSearchOrdenPView(), this.componentSearchProductoView = new app.ComponentSearchProductoView(), 
