...
!function($, window, document, undefined) {
    ...
    app.CreateOrdenp2View = Backbone.View.extend({
        ...
        initialize: function(opts) {
            opts !== undefined && _.isObject(opts.parameters) && (this.parameters = $.extend({}, this.parameters, opts.parameters)), 
            this.$wraperForm = this.$("#render-form-orden-producto"), this.listenTo(this.model, "change", this.render), 
            this.listenTo(this.model, "sync", this.responseServer), this.listenTo(this.model, "request", this.loadSpinner);
        },
        render: function() {
            var attributes = this.model.toJSON();
            ...
            this.$inputKey2 = this.$("#orden2_key2"), this.ready();
        },
    ...
    app.DespachopOrdenListView = Backbone.View.extend({
        ...
        addOne: function(despachopModel) {
            ...
            despachopModel.view = view, this.$el.prepend(view.render().el);
        },
        addAll: function() {
            this.collection.forEach(this.addOne, this);
        },
    ...
    app.MainOrdenesView = Backbone.View.extend({
        ...
        initialize: function() {
            ...
            this.$searchordenpEstado = this.$("#searchordenp_ordenp_estado"), this.ordersSearchTable = this.$ordersSearchTable.DataTable({
                dom: "<'row'<'col-sm-12'tr>><'row'<'col-sm-5'i><'col-sm-7'p>>",
                processing: !0,
                serverSide: !0,
                language: window.Misc.dataTableES(),
        ...
        clear: function(e) {
            e.preventDefault(), this.$searchordenpOrden.val(""), this.$searchordenpTercero.val(""), 
            this.$searchordenpTerceroName.val(""), this.$searchordenpEstado.val(""), this.ordersSearchTable.ajax.reload();
        }
    });
}(jQuery, this, this.document), app || (app = {}), function($, window, document, undefined) {
    app.ProductopOrdenListView = Backbone.View.extend({
        ...
        addOne: function(ordenp2Model) {
            ...
            ordenp2Model.view = view, this.$el.prepend(view.render().el), this.totalize();
        },
        addAll: function() {
            this.collection.forEach(this.addOne, this);
        },
        ...
        totalize: function() {
            ...
            this.$iva.length && this.$iva.html(window.Misc.currency(data.iva)), this.$total.length && this.$total.html(window.Misc.currency(data.total));
        },
        loadSpinner: function(target, xhr, opts) {
            window.Misc.setSpinner(this.parameters.wrapper);
        },
    ...
    app.ProductopOrdenList = Backbone.Collection.extend({
        ...
        totalize: function() {
            var unidades = this.unidades(), subtotal = this.subtotal(), iva = .16 * subtotal, total = subtotal + iva;
            return {
                unidades: unidades,
                subtotal: subtotal,
                iva: iva,
