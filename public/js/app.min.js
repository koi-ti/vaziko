/*! koi-frontend - v1.0.0 - 2017-06-30 */
!function($, window, document, undefined) {
    ...
    Misc.prototype = {
        ...
        clearForm: function(form) {
            form.find(":input").each(function() {
                ...
                field_type.hasClass("select2-default-clear") || field_type.hasClass("select2-default") ? field_type.select2("destroy").val("").select2() : field_type.hasClass("choice-select-autocomplete") && (field_type.empty(), 
    ...
    app.CreateCotizacionView = Backbone.View.extend({
        ...
        initialize: function(opts) {
            opts !== undefined && _.isObject(opts.parameters) && (this.parameters = $.extend({}, this.parameters, opts.parameters)), 
            this.detalleCotizacion2 = new app.DetalleCotizacion2List(), this.detalleCotizacion3 = new app.DetalleCotizacion3List(), 
            this.listenTo(this.model, "change", this.render), this.listenTo(this.model, "sync", this.responseServer), 
            this.listenTo(this.model, "request", this.loadSpinner);
        },
        render: function() {
            var attributes = this.model.toJSON();
            attributes.edit = !1, this.$el.html(this.template(attributes)), this.$wrapperDetalle = this.$("#render-detalle"), 
            this.$wrapperDetalle.html(this.templateFp({})), this.spinner = this.$("#spinner-main"), 
            this.referenceViews(), this.ready();
        },
        ready: function() {
            "function" == typeof window.initComponent.initToUpper && window.initComponent.initToUpper(), 
        ...
        onStore: function(e) {
            if (!e.isDefaultPrevented()) {
                e.preventDefault();
                var data = window.Misc.formToJson(e.target);
                data.cotizacion2 = this.detalleCotizacion2.toJSON(), this.model.save(data, {
                    patch: !0,
                    silent: !0
                });
            }
        ...
        addProducto: function(e) {
            console.log("Producto" == this.$(e.target).text().trim() ? "gaga" : "hoho");
        },
        loadSpinner: function(model, xhr, opts) {
            window.Misc.setSpinner(this.spinner);
        },
        responseServer: function(model, resp, opts) {
            if (window.Misc.removeSpinner(this.spinner), !_.isUndefined(resp.success)) {
                var text = resp.success ? "" : resp.errors;
                if (_.isObject(resp.errors) && (text = window.Misc.parseErrors(resp.errors)), !resp.success) return void alertify.error(text);
                window.Misc.redirect(window.Misc.urlFull(Route.route("cotizaciones.show", {
    ...
    app.DetalleCotizacion3ListView = Backbone.View.extend({
        ...
        addOne: function(cotizacion3Model) {
            ...
            cotizacion3Model.view = view, this.$el.prepend(view.render().el);
        },
        addAll: function() {
    ...
    app.DetalleCotizacion2ListView = Backbone.View.extend({
        ...
        addOne: function(cotizacion2Model) {
            ...
            cotizacion2Model.view = view, this.$el.prepend(view.render().el);
        },
        addAll: function() {
            this.$el.find("tbody").html(""), this.collection.forEach(this.addOne, this);
        },
    ...
    app.CreateTipoMaterialpView = Backbone.View.extend({
        ...
        responseServer: function(model, resp, opts) {
            if (window.Misc.removeSpinner(this.el), !_.isUndefined(resp.success)) {
                var text = resp.success ? "" : resp.errors;
                if (_.isObject(resp.errors) && (text = window.Misc.parseErrors(resp.errors)), !resp.success) return void alertify.error(text);
                window.Misc.cleanForm(this.$("#form-tipomaterialp")), window.Misc.redirect(window.Misc.urlFull(Route.route("tiposmaterialp.index")));
            }
        }
    });
}(jQuery, this, this.document), app || (app = {}), function($, window, document, undefined) {
